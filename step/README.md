...
tagline: |
  step: a cross-platform tool for RSA, ECDSA, JWT, JOSE, and general asymmetric encryption
---

To update or switch versions, run `webi step@stable`.

## Cheat Sheet

> step is like JWT.io, at your fingertips.
...
```bash
# step gen -key <key.format> -pub <pub.format>
step gen -key key.jwk.json -pub pub.jwk.json
```

JWK is the default format, for which you can use stdout (key) and stderr (pub)

```bash
step gen > key.jwk.json 2> pub.jwk.json
```

### How to generate PEM (PKCS) keys

```bash
step gen -key key.pem -pub pub.pem
```

Or DER

```bash
step gen -key key.der -pub pub.der
```

### How to sign a payload

```bash
# step sign --exp 1h <priv key> <data or file> > token.jwt 2> sig.jws
step sign --exp 1h key.jwk.json '{ "sub": "me@example.com" }' > token.jwt 2> sig.jws
```
...
```bash
# step verify <pub key> <signed file or data>
step verify pub.jwk.json token.jwt
```
...
`step` will generate either ECDSA or RSA at random and only support
